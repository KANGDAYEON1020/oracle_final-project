...
"""
08_query_hybrid.py
- BM25 + Vector(ChromaDB) 하이브리드 검색
- 메타데이터 필터링 (publisher/year/disease/allowed_use)
- RRF(Reciprocal Rank Fusion) 또는 가중합 방식 지원

사용법:
    python scripts/08_query_hybrid.py --q "EHEC 의심 소견과 주의점" --topk 5
    python scripts/08_query_hybrid.py --q "급성 설사 항생제" --publisher KSID --topk 10
    python scripts/08_query_hybrid.py --q "검사 의뢰 흐름" --mode rrf --topk 5
    python scripts/08_query_hybrid.py --q "탈수 치료" --alpha 0.4 --topk 5
...
def search_vector(query: str, collection, model: SentenceTransformer, 
    ...
    """Vector 검색, (chunk_id, score, meta) 리스트 반환"""
...
def main():
    ...
    if not chroma_dir.exists():
        raise FileNotFoundError(f"Missing ChromaDB: {chroma_dir}")
    ...
    # 2. ChromaDB 로드
    print(f"Loading ChromaDB and embedding model...")
    ...
    vector_results = search_vector(args.q, collection, model, topn=args.topn)
    ...
    for chunk_id, score, doc in hybrid_results:
        ...
        # BM25 doc 구조와 vector meta 구조 통합 처리
        if not meta:
            meta = {
